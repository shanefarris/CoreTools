...
namespace AssetManager
{
    public partial class Main : Form
    {
        ...
        private static void SetTabs(TextBox box)
        {
        ...
        private void PoplateControls()
        {
            ...
            cboAssetCategory.DataSource = Category.GetAllCategories().Select(c => new { c.Name, c.CategoryId }).ToList();
            cboAssetCategory.DisplayMember = "Name";
            cboAssetCategory.ValueMember = "CategoryId";
        ...
        /// <summary>
        /// Organizes the asset types into the correct lists passed based on the master asset list given.
        /// </summary>
        private void SeparateAssetDirList(IEnumerable<string> totalList, ref List<string> meshList, ref List<string> matList, ref List<string> programList,  ref List<string> otherList)
        {
        ...
        private void btnAssetAdd_Click(object sender, EventArgs e)
        {
            ...
            var categoryId = (int)cboAssetCategory.SelectedValue;
            ...
            if (extension == "mesh")
                ...
                result = Dal.Program.Create(txtAssetName.Text, categoryId, ref errorList);
            else
        ...
        private void btnAssetOpenDir_Click(object sender, EventArgs e)
        {
            ...
            if (Directory.Exists(txtAssetDirectory.Text))
            {
                fd.SelectedPath = txtAssetDirectory.Text;
            }
        ...
        private void btnAssetAddDir_Click(object sender, EventArgs e)
        {
            ...
            SeparateAssetDirList(fileList, ref meshList, ref matList, ref programList, ref otherList);

            // Save other files first
            otherList.ForEach(file => Asset.Create(file, categoryId, ref invalidList));

            // Save programs
            programList.ForEach(file => Dal.Program.CreateFromFile(file, categoryId, ref invalidList));

            // Save material files next
            matList.ForEach(file => Material.Create(file, categoryId, ref invalidList));

            // Save mesh files last
            meshList.ForEach(file => Mesh.Create(file, categoryId, ref invalidList));
        ...
        private void findMaterialDuplicatesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MaterialAsset.FindDuplicates(ref report);
            };
            ...
            backgroundWorker.RunWorkerCompleted += (s, args) =>
            {
                ...
                report.ForEach(r => txtAssetReport.Text += r + Environment.NewLine);
            };
        ...
        private void cboProfileExisting_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (profileName.Length > 0)
            {
                ...
                // Populate other assets that do not have any children assigned to them.
        ...
        {
            // Check to see if a profile is selected
            if (cboArchiveAssetProfiles.SelectedItem == null)
            {
                MessageBox.Show(@"Select a profile first.");
                return;
            }
