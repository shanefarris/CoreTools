	...
	{		
...

		{
			...
			catch(Exception &e) 
			{
				if (e.getNumber() != 4) 
				{
					cout << "\n|= ArtifexTerra3D =| Zoneloader 0.90: Error " << e.getNumber() << " creating Entity " << entName.c_str() << ": " << e.getFullDescription().c_str() << "\n";
				} 
				else if (e.getNumber() == 4) 
				{
					cout << "\n|= ArtifexTerra3D =| Zoneloader 0.90: Entity with the name " << entName.c_str() << " already exists, creating a random name:";
			...
			//****************************					
			{
				...
				try {
					...
					cout << "\n|= ArtifexTerra3D =| Zoneloader 0.90 SQLite: spawnloader error - problems creating " << spawn.name.c_str() << ":" << e.what() << "\n";
				};
			}
			//***************************			
		}		
